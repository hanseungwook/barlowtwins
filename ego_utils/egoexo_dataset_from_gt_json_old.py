...
from tests import visualization_util
...
import math
from train.augmentation_util import colorjitter_augmentation
...
try:
    ...
    print("hands23 not found, skipping")
    results_from_hands23 = None
import random
from torchvision.transforms.functional import to_pil_image
from camera_util import check_np_corrupted
from data_processing import ik_util, mano_util
...
class EgoExoDatasetFromGTJson(Dataset):
    ...
    def __init__(self, release_dir, 
                ...
                rectified_ego_height=1404,
                rectified_ego_width=1404,
                load_cam_data=True,
                length_scale_factor=1,
                text_annotation_json_path=None,
                max_period_to_associate_timestamp=2,
                sample_clips_with_text_annotations_only=False,
                load_mano_params_from_cache=False):
        ...
        length_scale_factor: artificially scale the dataset length so we can utilize the fact that workers try to preload the epoch
        """
        ...
        if text_annotation_json_path is not None:
            with open(text_annotation_json_path, "r") as fp:
                self.text_annotation_json_loaded = json.load(fp)
                # text sampling rules:
                # since the annotation provides a single annotated timestamp,
                # for any specific timestep, we associate closest future annotated timestamp to the timestep, as long as the future timestamp is no more than 5 seconds in the future
                self.max_period_to_associate_timestamp = max_period_to_associate_timestamp
                self.sample_clips_with_text_annotations_only = sample_clips_with_text_annotations_only
        else:
            self.sample_clips_with_text_annotations_only = False

        self.load_mano_params_from_cache = load_mano_params_from_cache
        count = 0
        ...
        self.rectified_ego_height = rectified_ego_height
        self.rectified_ego_width = rectified_ego_width

    def set_joints_wrt_cam_normalization_from_take_names(self, take_names):
        """
        Calculates and sets the normalization statistics for the dataset for a given action type
        """

        x_wrt_cam_collected = []

        assert self.three_d_keypoints_torch_root is not None
        for take_name in take_names:
            if os.path.exists(os.path.join(self.joint_wrt_cam_cache_dir, f"joints_wrt_cam_valid_{take_name}.pt")):
                ...
                x_wrt_cam_collected.append(joints_wrt_cam_valid)
                continue
            ...
            joints_wrt_world_clips = three_d_keypoints_torch['nonlinear_results_slerped'][:, 1, :].detach().contiguous().as_strided(size=(self.take_to_take_num_frames[take_name] - self.horizon, self.horizon, 21, 3), 
                                                                                                        stride=(21*3, 21*3, 3, 1))
            
            joints_wrt_world_homo_clips = torch.cat([joints_wrt_world_clips, torch.ones((*joints_wrt_world_clips.shape[:3], 1))], axis=-1)
            # TODO: also filter... we only want slerp bool true frames!

            # -> num_frames - H, H, 21, 4
            # reshape them to be clips
            joints_wrt_cam_clips = torch.einsum("bhjd, bid -> bhji",  joints_wrt_world_homo_clips, torch.linalg.inv(aria_cam_wrt_world)[:self.take_to_take_num_frames[take_name] - self.horizon])[..., :3]

            valid_keys_boolean = torch.sum(three_d_keypoints_torch['slerp_boolean'][:, 1].detach().contiguous().as_strided(size=(self.take_to_take_num_frames[take_name]-self.horizon, self.horizon), stride=(1, 1)), axis=-1) == self.horizon

            joints_wrt_cam_valid = joints_wrt_cam_clips[valid_keys_boolean]
        ...
        return x_wrt_cam_collected

    def set_pseudogripper_10d_normalization_from_take_names(self, take_names):
        """
        Calculates and sets the normalization statistics for the dataset for a given action type
        """
        proprio_wrt_cam_collected = []
        action_wrt_cam_collected = []

        assert self.load_mano_params_from_cache
        for take_name in take_names:

            three_d_keypoints_torch = torch.load(os.path.join(self.three_d_keypoints_torch_root, take_name, "serialization_compiled.pt"))
            valid_keys_boolean = torch.sum(three_d_keypoints_torch['slerp_boolean'][:, 1].detach().contiguous().as_strided(size=(self.take_to_take_num_frames[take_name]-self.horizon, self.horizon), stride=(1, 1)), axis=-1) == self.horizon

            # -> num_total_samples, 4, 4
            aria_cam_wrt_world = torch.from_numpy(np.load(self.take_name_to_aria_cam_wrt_world_save_path[take_name])).to(torch.float32)

            # -> num_frames, 2, 109
            # mano_params_wrt_cam = torch.load(os.path.join(self.three_d_keypoints_torch_root, take_name, "ik_results_wilorinitTrue.pt"))['mano_params']
            mano_params_wrt_cam = torch.load(os.path.join(self.three_d_keypoints_torch_root, take_name, "ik_results_final_wilorinitTrue_chunksize20.pt"))['mano_params']

            # TODO: 
            # 1. calculate pseudogripper wrt cam
            # 2. calculate pseudogripper wrt world
            # 3. convert to clips centered on the current cam frame
            # flatten the clips and compute min/max

            # including invalid samples
            num_total_samples = three_d_keypoints_torch['slerp_boolean'].shape[0]
            pseudogripper_10d_wrt_cam_ph = torch.zeros((num_total_samples, 10))

            slerp_valid_pseudogripper_10d_wrt_cam = ik_util.get_pseudogripper_10d_wrt_cam_from_mano(mano_params_wrt_cam[three_d_keypoints_torch['slerp_boolean'][:, 1].clone(), 1, :], mano_util.mano[1], True)

            pseudogripper_10d_wrt_cam_ph[three_d_keypoints_torch['slerp_boolean'][:, 1].detach()] = slerp_valid_pseudogripper_10d_wrt_cam

            # transform with cam wrt world
            pseudogripper_10d_wrt_world_ph = ik_util.apply_left_transform_to_10d(pseudogripper_10d_wrt_cam_ph, aria_cam_wrt_world)

            # -> num_subclips, horizon, 10
            pseudogripper_10d_wrt_world_clips = pseudogripper_10d_wrt_world_ph.detach().contiguous().as_strided(size=(self.take_to_take_num_frames[take_name] - self.horizon, self.horizon, 10), stride=(10, 10, 1))

            aria_world_wrt_cam_clips = torch.linalg.inv(aria_cam_wrt_world).detach().contiguous().as_strided(size=(self.take_to_take_num_frames[take_name] - self.horizon, self.horizon, 4, 4), stride=(16, 16, 4, 1))

            pseudogripper_10d_wrt_cam_clips = ik_util.apply_left_transform_to_10d(pseudogripper_10d_wrt_world_clips.reshape(-1, 10), aria_world_wrt_cam_clips.reshape(-1, 4, 4)).reshape(self.take_to_take_num_frames[take_name] - self.horizon, self.horizon, 10)

            pseudogripper_10d_wrt_cam_clips_valid = pseudogripper_10d_wrt_cam_clips[valid_keys_boolean]
            proprio_wrt_cam_collected.append(pseudogripper_10d_wrt_cam_clips_valid)

            action = pseudogripper_10d_wrt_cam_clips_valid.detach().clone()

            # TODO: change this
            action[:, :, -1] = -1


            action_wrt_cam_collected.append(action)

        # concatenate along the batch dimension then reshape to collapse the horizon dimension
        proprio_wrt_cam_collected_tensor = torch.cat(proprio_wrt_cam_collected, axis=0).reshape(-1, 10)
        self.proprio_min_wrt_cam_running = torch.quantile(proprio_wrt_cam_collected_tensor, q=0.01, dim=0)
        self.proprio_max_wrt_cam_running = torch.quantile(proprio_wrt_cam_collected_tensor, q=.99, dim=0)

        # manually set the grasp action min/max
        action_wrt_cam_collected_tensor = torch.cat(action_wrt_cam_collected, axis=0).reshape(-1, 10)
        self.proprio_min_wrt_cam_running[-1] = 0
        self.proprio_max_wrt_cam_running[-1] = 70

        self.action_min_wrt_cam_running = self.proprio_min_wrt_cam_running.detach().clone()
        self.action_max_wrt_cam_running = self.proprio_max_wrt_cam_running.detach().clone()
        self.action_min_wrt_cam_running[-1] = -1
        self.action_max_wrt_cam_running[-1] = +1

        assert torch.all(self.action_max_wrt_cam_running - self.action_min_wrt_cam_running != 0)
        assert torch.all(self.proprio_max_wrt_cam_running - self.proprio_min_wrt_cam_running != 0)
        # if min equals the max... what happens for normalization?
        return proprio_wrt_cam_collected_tensor, action_wrt_cam_collected_tensor
    ...
    def __len__(self):
        if self.allow_takes_with_annotations_only:
            # assuming right now that there are small number of annotations per take
        ...
        if self.frames_type_to_use == "iterate_all_frames":
            ...
            # this is the standard training setting
            return len(self.selected_take_names) * self.length_scale_factor
    ...
    def __getitem__(self, sample_idx):
        if self.frames_type_to_use == "iterate_all_frames" and not self.allow_takes_with_annotations_only:
            ...
            # modulo here because for efficiency reasons we want the dataset length to be much larger than the number of videos sometimes
            # i.e. to avoid the expense of recreating the dataloader
        ...
        if self.cached_rgb_dir:
            try:
                ...
                print("Unable to load cached video decoder")
        ...
        if self.three_d_keypoints_torch_root is not None:
            ...
            assert self.cached_rgb_dir is not None, "Cached rgb dir must be set if three d keypoints torch root is set"
        else:
            three_d_keypoints_torch = None

        if self.load_mano_params_from_cache:
            # mano_params_torch = torch.load(os.path.join(self.three_d_keypoints_torch_root, take_name, "ik_results.pt"))
            mano_params_torch = torch.load(os.path.join(self.three_d_keypoints_torch_root, take_name, "ik_results_final_wilorinitTrue_chunksize20.pt"))
        else:
            mano_params_torch = None

        if self.load_mano_params_from_cache and self.three_d_keypoints_torch_root is not None:
            n1 = torch.sum(torch.sum(torch.abs(three_d_keypoints_torch['nonlinear_results_slerped'][:, 1, :, :]), axis=[1, 2]) == 0).item()
            n2 = torch.sum(torch.sum(torch.abs(mano_params_torch['mano_params'][:, 1, :]), axis=[1]) == 0).item()
            assert n1 == n2, f"Number of zero 3d kps for right hand: {n1} != Number of zero mano params for right hand: {n2}"
        ...
        if os.path.exists(annotation_json_path) and self.allow_takes_with_annotations_only:
            ...
            if self.frames_type_to_use == "iterate_all_frames":
                ...
                # -> num_frames - H, H
                # the 0th clip is the 0th frame
                # we cut off horizon because we don't want to have a clip start at the very end when there are not enough frames to make a horizon
                slerp_bool_strided = three_d_keypoints_torch['slerp_boolean'][:, 1].detach().contiguous().as_strided(size=(self.take_to_take_num_frames[take_name]-self.horizon, self.horizon), stride=(1, 1))

                # -> num_frames - H, H, 21, 3
                nonlinear_joints_strided = three_d_keypoints_torch['nonlinear_results_slerped'][:, 1, ...].detach().contiguous().as_strided(size=(self.take_to_take_num_frames[take_name]-self.horizon, self.horizon, 21, 3), stride=(21*3, 21*3, 3, 1))
                ...
                valid_keys_boolean = horizon_bool

                if self.filter_clip_proprio_norm is not None:
                    norm_bool = nonlinear_joints_norm_strided >= self.filter_clip_proprio_norm
                    valid_keys_boolean = valid_keys_boolean * norm_bool

                if self.sample_clips_with_text_annotations_only:
                    # the only valid sampled frame timesteps are timesteps that are at most 5 seconds away from a text annotation timestep
                    # this ensures we always have a text annotation per clip
                    text_timestamps_seconds = [float(_) for _ in self.text_annotation_json_loaded[take_name].keys()]
                    # filter out any timestamps that are less than horizon away from the end of video
                    text_timestamps_30fps = torch.Tensor([int(_ * 30) for _ in text_timestamps_seconds if int(_ *30) <= self.take_to_take_num_frames[take_name] - self.horizon])
                    text_timestamps_30fps_with_prefixes = ((text_timestamps_30fps - self.max_period_to_associate_timestamp * 30).unsqueeze(1) + torch.arange(self.max_period_to_associate_timestamp * 30).unsqueeze(0)).flatten().unique()

                    text_clip_bool = torch.zeros((self.take_to_take_num_frames[take_name]-self.horizon,), dtype=torch.bool)
                    text_clip_bool[text_timestamps_30fps_with_prefixes.int()] = True

                    valid_keys_boolean = valid_keys_boolean * text_clip_bool
                ...
                if self.sample_clips_with_text_annotations_only:
                    text_labels = []
                    # for every sampled clip start time, find the closest *future* text annotation timestamp
                    for sampled_frame_idx in sampled_frame_idxs_30fps:
                        sampled_frame_idxs_in_seconds = sampled_frame_idx / 30
                        # find the closest future text annotation timestamp
                        closest_future_text_timestamp = np.array(text_timestamps_seconds)[np.array(text_timestamps_seconds) >= sampled_frame_idxs_in_seconds.item()][0]
                        text_labels.append(self.text_annotation_json_loaded[take_name][str(closest_future_text_timestamp)])
        ...
        if self.return_full_images:
            ...
            rgb_frames = torch.zeros((num_placeholder_frames, self.rectified_ego_height, self.rectified_ego_width, 3), dtype=torch.uint8)
            rgb_debug_frames = torch.zeros((num_placeholder_frames, self.rectified_ego_height, self.rectified_ego_width, 3), dtype=torch.uint8)
        ...
        mano_params_tensor = torch.zeros((num_placeholder_frames, self.horizon, 2, 109), dtype=torch.float32)
        ...
        if self.three_d_keypoints_torch_root is not None:
            ...
            # assert that none of the individual actions are all zeros... that would not make sense
            if self.frames_type_to_use == "valid_3d_kp_clips":
                assert not torch.any(torch.sum(torch.abs(ego_keypoint_3d_wrtworld_tensor[:, :, 1, :, :]), axis=[2, 3]) == 0.0)


        if self.load_mano_params_from_cache:
            # -> batch_size, horizon + 1, 109
            mano_params_tensor[:, :, 1, :] = mano_params_torch['mano_params'][:, 1, :][sampled_frame_idxs_30fps[:, None] + torch.arange(self.horizon)[None, :]]

            if self.frames_type_to_use == "valid_3d_kp_clips":
                assert not torch.any(torch.sum(torch.abs(mano_params_tensor[:, :, 1, :]), axis=-1) == 0.0)
        else:
            mano_params_tensor = None
        ...
        if self.return_full_images:
            ...
            for local_frame_idx in range(num_sampled_frames_for_this_video):
                if cam_name == self.take_name_to_egocam_name[take_name]:
                    ...
                    # so the intrinsics parameters in take_name_to_egocam_fisheye_rgb_calibation are for 512x512 / 150
                    # but since our original source image is now 1404, we need to scale the focal length by 1404/512
                    # rectified_array, principal_points, focal_lengths = undistort_aria_given_device_calib(
                    #     self.take_name_to_egocam_fisheye_rgb_calibration[take_name], cv2_fisheye_rgb_data[local_frame_idx], "camera-rgb", self.rectified_ego_focal_length, full_image_size[0], full_image_size[0])
        ...
        for local_idx, frame_idx in enumerate(sampled_frame_idxs_30fps.tolist()):
            ...
            # below loop processes the bboxes, only needed if the rgb cache dir is not set
            if self.cached_rgb_dir is not None:
                if self.bbox_npy_root is not None:
                    if frame_idx in bbox_json_for_take[cam_name]['all_hands_found_boolean_across_frames'].keys():
                        ...
                        # assert np.sum(np.concatenate([_ for _ in bbox_json_for_take['aria04']['all_hands_found_across_frames'].values()], axis=0)) != 0, "No hands found in bbox json"
        ...
        Build aria intrinsic manually
            Note: this intrinsics is always for the full image but with a modified focal length
        """
        ...
        # mano_params_tensor: batch_size, horizon, 2, 109
        # select just the right hand, flatten it, then return and reshape

        aria_loaded = torch.from_numpy(np.load(self.take_name_to_aria_cam_wrt_world_save_path[take_name])).float()

        # -> batch_size, horizon, 4, 4
        if self.frames_type_to_use == "iterate_all_frames":
            aria_cam_wrt_world = aria_loaded[sampled_frame_idxs_30fps[:, None]]
        else:
            aria_cam_wrt_world = aria_loaded[sampled_frame_idxs_30fps[:, None] + torch.arange(self.horizon)[None, :]]

        if self.load_mano_params_from_cache:
            # -> batch_size, horizon, 1, 10
            pseudogripper_wrt_cam_10d_proprio = ik_util.get_pseudogripper_10d_wrt_cam_from_mano(mano_params_tensor[:, :, 1, :].view(-1, 109), mano_util.mano[1], True).view(self.num_subclips_per_video, self.horizon, 1, 10)
            # add some dummy zeros to make it the right shape

            pseudogripper_wrt_cam_10d_action = ik_util.get_pseudogripper_10d_wrt_cam_from_mano(mano_params_tensor[:, :, 1, :].view(-1, 109), mano_util.mano[1], False).view(self.num_subclips_per_video, self.horizon, 1, 10)

            # -> batch_size, horizon, 1, 10
            pseudogripper_wrt_world_10d_proprio = ik_util.apply_left_transform_to_10d(pseudogripper_wrt_cam_10d_proprio.view(-1, 10), aria_cam_wrt_world.view(-1, 4, 4)).view(self.num_subclips_per_video, self.horizon, 1, 10)
            pseudogripper_wrt_world_10d_action = ik_util.apply_left_transform_to_10d(pseudogripper_wrt_cam_10d_action.view(-1, 10), aria_cam_wrt_world.view(-1, 4, 4)).view(self.num_subclips_per_video, self.horizon, 1, 10)

            # -> batch_size, horizon, 2, 10
            pseudogripper_wrt_world_10d_proprio = torch.cat([torch.zeros_like(pseudogripper_wrt_world_10d_proprio), pseudogripper_wrt_world_10d_proprio], dim=2)
            pseudogripper_wrt_world_10d_action = torch.cat([torch.zeros_like(pseudogripper_wrt_world_10d_action), pseudogripper_wrt_world_10d_action], dim=2)

        # pseudogripper_wrt_cam_10d_proprio = torch.cat([torch.zeros_like(pseudogripper_wrt_cam_10d_proprio), pseudogripper_wrt_cam_10d_proprio], dim=2)
        # pseudogripper_wrt_cam_10d_action = torch.cat([torch.zeros_like(pseudogripper_wrt_cam_10d_action), pseudogripper_wrt_cam_10d_action], dim=2)

        out_dict = dict(
            sampled_frame_idxs_30fps=sampled_frame_idxs_30fps,
            take_name=[take_name] * num_sampled_frames_for_this_video,
            cam_name=[cam_name] * num_sampled_frames_for_this_video,
            text_labels=text_labels if self.sample_clips_with_text_annotations_only else dict(), #TODO: maybe we should enable text label sampling even when a clip doesn't have text labels
            ...
            ego_keypoint_2d_fullimg_tensor=ego_keypoint_2d_fullimg_tensor, # this will be empty if the bbox doesn't have the 2d keypoints
            ...
            rgb_patches_tensor=rgb_patches_tensor if not self.cached_rgb_dir else dict(),
            ...
            aria_cam_wrt_world=aria_cam_wrt_world if self.load_cam_data else {}, # this line may fail if you are launching many jobs and they keep rewriting each other
            ...
            mano_params_tensor=mano_params_tensor if self.load_mano_params_from_cache else dict(),
            # pseudogripper_wrt_cam_10d_proprio=pseudogripper_wrt_cam_10d_proprio if self.load_mano_params_from_cache else dict(),
            # pseudogripper_wrt_cam_10d_action=pseudogripper_wrt_cam_10d_action if self.load_mano_params_from_cache else dict()
            pseudogripper_wrt_world_10d_proprio=pseudogripper_wrt_world_10d_proprio if self.load_mano_params_from_cache else dict(),
            pseudogripper_wrt_world_10d_action=pseudogripper_wrt_world_10d_action if self.load_mano_params_from_cache else dict()
        )
